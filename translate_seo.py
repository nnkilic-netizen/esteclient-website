...
# We will just replace seoAlts array directly if it exists
seo_tr = """const seoAlts = [
...
text = text.replace(seo_tr, seo_new)

# Find where seoAlts[i] is used
text = text.replace("alt={seoAlts[i] || 'Müşteri takip programı'}", "alt={(language === 'tr' ? seoAltsTr[i] : language === 'en' ? seoAltsEn[i] : seoAltsFr[i]) || 'Müşteri takip programı'}")
