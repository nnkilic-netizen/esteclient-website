...
export function BeautyCrmSite() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <nav className="sticky top-0 z-40 border-b border-border/70 bg-background/90 backdrop-blur-md" aria-label="Ana navigasyon">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <div className="hidden items-center gap-3 md:flex">
            ...
            <a href="/demo" className="rounded-full bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground transition-transform hover:-translate-y-0.5" onClick={handleDemoClick}>{language === 'tr' ? 'Ücretsiz Deneyin (Demo Sürümü)' : 'Try for free (Demo)'} <ArrowRight className="ml-1 inline size-4" /></a>
          </div>
      ...
      <button className="fixed bottom-5 right-5 z-50 flex size-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-xl shadow-primary/25 transition-transform hover:scale-105" aria-label={language === 'tr' ? 'Online mesajlaşmayı aç' : 'Open online chat'} onClick={() => setChatOpen(true)}><MessageCircle className="size-6" /></button>
      {chatOpen && <aside className="fixed bottom-5 right-5 z-50 w-[min(calc(100vw-2rem),360px)] overflow-hidden rounded-3xl border border-border bg-card shadow-2xl" aria-label={language === 'tr' ? 'Online mesajlaşma' : 'Online chat'}><div className="flex items-center justify-between bg-primary px-5 py-4 text-primary-foreground"><div><p className="font-semibold">{language === 'tr' ? 'EsteClient destek' : 'EsteClient support'}</p><p className="text-xs text-primary-foreground/70">{language === 'tr' ? 'Genellikle birkaç dakika içinde yanıtlarız' : 'Usually replies in a few minutes'}</p></div><button aria-label={language === 'tr' ? 'Mesajlaşmayı kapat' : 'Close chat'} onClick={() => setChatOpen(false)}><X className="size-5" /></button></div><div className="flex min-h-52 flex-col gap-3 p-4"><div className="max-w-[85%] rounded-2xl rounded-tl-sm bg-secondary px-3 py-2 text-sm leading-5">{language === 'tr' ? 'Merhaba. EsteClient hakkında nasıl yardımcı olabilirim?' : 'Hello. How can I help you with EsteClient?'}</div>{sent && <div className="ml-auto max-w-[85%] rounded-2xl rounded-tr-sm bg-primary px-3 py-2 text-sm text-primary-foreground">{language === 'tr' ? 'Mesajınızı aldık. Ekibimiz kısa süre içinde size dönüş yapacak.' : 'We received your message. Our team will get back to you shortly.'}</div>}</div><div className="flex gap-2 border-t border-border p-3"><input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.nativeEvent.isComposing && event.keyCode !== 229) sendMessage() }} className="min-w-0 flex-1 rounded-full border border-input bg-background px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" placeholder={language === 'tr' ? 'Mesajınızı yazın...' : 'Type your message...'} aria-label={language === 'tr' ? 'Mesajınız' : 'Your message'} /><button onClick={sendMessage} className="flex size-10 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground" aria-label={language === 'tr' ? 'Mesaj gönder' : 'Send message'}><Send className="size-4" /></button></div></aside>}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 animate-in fade-in duration-200">
          <div className="bg-background p-6 rounded-2xl max-w-sm w-full shadow-2xl text-center">
            ...
            <div className="mt-6 flex flex-col gap-3">
              <a href="/demo" onClick={() => setMobileWarningOpen(false)} className="w-full block rounded-full bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground text-center transition-transform hover:-translate-y-0.5">
