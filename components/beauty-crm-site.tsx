    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="top" className="relative mx-auto flex max-w-7xl flex-col items-center text-center px-5 pb-20 pt-16 lg:px-8 lg:pb-24 lg:pt-24 overflow-x-hidden">
        <div className="relative z-10 flex flex-col items-center">
          ...
          <h1 className="max-w-4xl text-balance text-4xl font-semibold tracking-tight sm:text-5xl lg:text-6xl lg:leading-[1.08]">{language === 'tr' ? <>İşletmeniz için <span className="text-primary">sade, kullanışlı, çoklu platform erişimli</span> ve güçlü müşteri takip programı.</> : language === 'en' ? <>A <span className="text-primary">simple, powerful</span> CRM for your business.</> : <>Un CRM <span className="text-primary">simple et puissant</span> pour votre entreprise.</>}</h1>
          <p className="mt-6 max-w-2xl text-pretty text-base leading-7 text-muted-foreground sm:text-lg">{language === 'tr' ? 'EsteClient; randevularınızı, müşterilerinizi, ürün ve hizmet satışlarınızı tek bir ekrandan yönetir. En güçlü yanı ise personel performansından gelir-gider analizine uzanan kapsamlı raporlama yapısıdır.' : language === 'en' ? 'EsteClient manages appointments, customers, product and service sales in one simple workspace. Track team performance and revenue-expense reports with clarity.' : 'EsteClient gère les rendez-vous, les clients et les ventes dans un espace de travail simple. Suivez les performances de l\'équipe et les rapports de revenus-dépenses avec clarté.'}</p>
          ...
          <div id="gallery-slider" className="w-full h-full flex snap-x snap-mandatory overflow-x-auto items-center [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {Array.from({ length: 18 }, (_, i) => i + 1).map((num, i) => {
              ...
              <div key={num} className="w-screen h-[100dvh] shrink-0 snap-center flex items-center justify-center p-4 sm:p-12">
                <img src={`/ss${num}.png`} alt={(language === 'tr' ? seoAltsTr[i] : language === 'en' ? seoAltsEn[i] : seoAltsFr[i]) || (language === 'tr' ? 'Müşteri takip programı' : language === 'en' ? 'Customer tracking program' : 'Programme de suivi client')} className="max-w-full max-h-full object-contain rounded-xl shadow-2xl" />
              </div>
            )})}
          </div>
        </div>
