...
import { X, UserPen, Upload, Image as ImageIcon } from "lucide-react"
...
export function EditUserModal({ isOpen, onClose, user, onSave, branches }: EditUserModalProps) {
  ...
  useEffect(() => {
    if (user) {
      ...
      setLocalImage(user.avatarUrl || null)
    }
  }, [user])
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-4 border-b border-border/50 bg-muted/30">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-foreground">Personel Düzenle</h2>
          </div>
          <button 
            onClick={onClose}
